...
export class AuthService {
  ...
  async login(email: string, password: string): Promise<any> {
    try {
      const response = await this.http
        .post<any>(`${environment?.baseUrl}/${endpoints?.login}`, {
          email,
          password,
        })
        .toPromise();
  ...
  public getRefreshToken() {
    ...
    if (timeDifference === 10) {
      //once time diffrence equals 10secs, the refresh token is called
      this.refreshToken();
    }
  }
  ...
  public refreshToken(): Observable<any> {
    return this.http
      .post<any>(`${environment?.baseUrl}/${endpoints?.refreshToken}`, {
  ...
  public forgotPasswords(path: forgotPasswords): Observable<any> {
    return this.http.post<any>(
      `${environment?.baseUrl}/${endpoints?.forgetPassword}/${path.identifier}`,
  ...
  public validateOTP(path: forgotPasswords | any): Observable<any> {
    ...
    return this.http.post<any>(
      `${environment?.baseUrl}/${endpoints?.validateForgetPasswordToken}?token=${path.token}&identifier=${path.identifier}`,
      { headers: this.interceptor?.customNoAuthHttpHeaders },
    );
  }
  ...
  public changePasswords(path: changePassword): Observable<any> {
    ...
    return this.http.post<any>(
      `${environment?.baseUrl}/${endpoints?.changePassword}`,
      body,
      { headers: this.interceptor?.customHttpHeaders },
    );
  }
  ...
  public getUserDetails(): Observable<any> {
    return this.http
      .get<any>(`${environment?.baseUrl}/${endpoints?.getUserDetails}`, {
        headers: this.interceptor?.customHttpHeaders,
      })
      .pipe(
        map((res: any) => {
  ...
  public logoutUser(): Observable<any> {
    return this.http.post<any>(
      `${environment?.baseUrl}/${endpoints?.logoutUser}`,
      {},
      { headers: this.interceptor?.customHttpHeaders },
    );
  }
  ...
  public setAgentToken(token: any) {
    localStorage.setItem('user', token);
    // Ensure the token is available immediately
    this.token = token;
  }
